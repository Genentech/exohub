...
func (m stashModel) helpView() (string, int) {
	...
	selectionHelp = append(selectionHelp, "space", "select", "d", "download", "t", "transfers")
...
func (m stashModel) miniHelpView(entries ...string) string {
	...
	var (
		maxWidth = m.common.width - stashViewHorizontalPadding - 2
		s        = " "
	)
	...
	for i := 0; i < len(entries); i = i + 2 {
		...
		btn := renderToolbarBtn(k, v)

		if ansi.PrintableRuneWidth(s)+ansi.PrintableRuneWidth(btn)+1 >= maxWidth {
			break
		}
...
// helpBarAction maps an X coordinate click on the mini help bar to the
// corresponding keyboard shortcut key. Returns "" if no match.
func (m stashModel) helpBarAction(x int) string {
	// Build the same entry list as miniHelpView
	numDocs := len(m.getVisibleMarkdowns())

	if m.filterState == filtering {
		return "" // filtering help bar has no clickable actions
	}

	var entries []string
	var navHelp, filterHelp, selectionHelp, editHelp, sectionHelp, appHelp []string
	...
	entries = concatStringSlices(navHelp, filterHelp, selectionHelp, editHelp, sectionHelp, appHelp)

	// Walk through entries computing cumulative X positions
	// Layout: " " (gutter=1) then [btn] [btn] [btn]...
	// Each button is rendered with toolbarBtnStyle which adds padding(0,1)
	pos := stashIndent + 1 // stash indent + left gutter

	for i := 0; i < len(entries)-1; i += 2 {
		key := entries[i]
		val := entries[i+1]
		btnW := ansi.PrintableRuneWidth(renderToolbarBtn(key, val))

		if x >= pos && x < pos+btnW {
			return key
		}
		pos += btnW + 1 // +1 for gap between buttons
	}
