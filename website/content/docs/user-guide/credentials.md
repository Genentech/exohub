...
### Clearing the S3 Credential Cache

The `exo-credential-helper` caches S3 credentials locally to avoid repeated API calls. If you experience access problems after a permission change, you can purge the cache manually:

```bash
exo-credential-helper --clear
```

This removes all cached credential files and lock files. You can also clear the cache automatically by running `exo login --force` or `exo logout`, both of which purge the S3 credential cache as part of the logout/re-authentication flow.
