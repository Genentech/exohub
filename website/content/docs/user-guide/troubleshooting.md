...
**5. Grants & S3 Access** *(per grants-enabled remote)* — For each remote with `grants: true`, runs `GetDataAccess READ` and `GetDataAccess READWRITE`. For READWRITE, the check also performs a real write probe (zero-byte PutObject + DeleteObject) to confirm the vended credentials carry actual write permission — not just that credentials were issued. Reports which principal is vended: a *location role* (write-capable) vs the identity role (read-only). This is the most diagnostic signal for read-vs-write problems.

Common fixes:
- No matching grant → `exo init` to provision, or ask an owner to grant access
- `GetDataAccess(READWRITE)` fails with "credentials vended but write probe denied" → the grant is scoped to READ only; owner must re-run `exo init` to provision a write grant
...
exo doctor
```
Check the **Grants & S3 Access** group. The `GetDataAccess(READWRITE)` check now includes a built-in write probe — if it fails with "write probe denied", the vended credentials don't have write permission. Ask a dataset owner to re-provision grants via `exo init`. For an end-to-end write verification that goes beyond grants, use `--write-test`.
