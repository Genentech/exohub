...
- `EXOHUB_JANUS_CLIENT_ID`: override Janus client ID for authentication (optional, for testing)
...
Authenticate with Roche SSO via Janus:
...
**Note**: The `login` command uses the Janus device flow, which allows you to authenticate on any device with a browser. A QR code is displayed for easy mobile authentication. AWS credentials are automatically fetched and stored for use with s5cmd.
